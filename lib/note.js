...
module.exports.edit = function(state){
	...
	mkdirp(dir, function(err){
		...
		editor.on('exit', function() {
			if(config.save.git_repository !== undefined){
 	...
 	async.waterfall([
 		function(callback){
 			Git.Repository.open(basedir).then(function(repo){
 				...
 				require('child_process').exec('bash ' + __dirname + '/init.sh ' +basedir+' '+repo_url, function(err, stdout, stderr){
 					console.log(stderr);
 					callback(null);
 				});
 			});
 			...
 			// add . commit push
 			require('child_process').exec('bash ' + __dirname + '/push.sh ' + basedir + ' ' + mkdate(), function(err, stdout, stderr){
